...
const InterviewsAdmin = () => {
  ...
  const handleExport = () => {
    toast.success("Interviews list exported successfully!");
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Appointed': return 'bg-purple-100 text-purple-800 border-purple-200';
                  ...
                  currentPagedData.map((item) => (
                    <tr key={item.id} className="border-b border-[var(--color-border)] hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="py-3.5 px-3 text-center">
                        {item.status === 'Appointed' || appointments.some(app => app.email.toLowerCase() === item.email.toLowerCase() && app.status === 'Approved') ? (
                          <span className={`px-2.5 py-0.5 rounded text-[10px] font-semibold border ${getStatusColor('Appointed')}`}>
                            Appointed
                          </span>
                        ) : (
                          <select
                            ...
                            className={`px-2 py-0.5 rounded text-[10px] font-semibold border focus:outline-none cursor-pointer ${getStatusColor(item.status)}`}
                          >
                            <option value="Scheduled">Scheduled</option>
                            <option value="In Progress">In Progress</option>
                            <option value="Completed">Completed</option>
                      ...
                      <td className="py-3.5 px-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center transition-colors cursor-pointer border border-[var(--color-border)] bg-[var(--color-main-bg)] text-slate-600 hover:text-slate-900 hover:bg-slate-50"
                            style={{ borderRadius: 'var(--radius-sm)' }}
                            title="Delete"
                          >
                            <FiTrash2 size={13} />
            ...
            <div className="flex flex-col gap-4 text-xs text-slate-700">
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-2">
                ...
                <div className="col-span-2">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-semibold inline-block mt-1 ${getStatusColor(selectedInterview.status)}`}>
